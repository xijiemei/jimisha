const Game = {
    ...
    updateAll: function() {
        ...
        if (this.mode === 'MP' && Net.isHost) this.scheduleStallWatchdog();
    },
    ...
    preloadAssets: function() {
        if (this.assetsPreloadStarted) return;
        this.assetsPreloadStarted = true;
        const imageUrls = new Set();
        Object.values(HEROES).forEach(h => h.img && imageUrls.add(h.img));
        Object.values(CARDS).forEach(c => c.img && imageUrls.add(c.img));
        const soundUrls = ['hiss.mp3', 'bark.mp3', 'happy.mp3', 'huh.mp3', 'banana.mp3', 'death-oh-no.mp3', 'tom.mp3']
        ...
        soundUrls.forEach(url => {
            ...
            audio.src = url;
            this.preloadedAssets.push(audio);
    ...
    askForResponse: function(type, sourceId, targetId, extra) {
        this.gameState.pendingAction = { type, sourceId, targetId, createdAt: Date.now(), ...extra };
        this.updateAll();
    },

    scheduleStallWatchdog: function() {
        ...
        if (state.pendingAction) {
            let target = state.players.find(p => p.id === state.pendingAction.targetId);
            if (!target || target.isBot || !target.alive || state.pendingAction.type === 'BOT_EXPLOSION_DEATH') return;
            let elapsed = Date.now() - (state.pendingAction.createdAt || Date.now());
            let delay = Math.max(800, this.responseTimeoutMs - elapsed);
            this.watchdogTimer = setTimeout(() => this.forceResolveTimeout(state.pendingAction), delay);
            return;
        }

        if (state.aoeState || state.initialActivePick || state.awaitTeamPick) return;
        let cur = state.players[state.turnIdx];
        if (!cur || cur.isBot || !cur.alive) return;
        let elapsed = Date.now() - (state.turnStartedAt || Date.now());
        let delay = Math.max(1000, this.turnTimeoutMs - elapsed);
        this.watchdogTimer = setTimeout(() => this.forceTurnTimeout(cur.id), delay);
    },

    forceResolveTimeout: function(pending) {
        if (!this.gameState.pendingAction || this.gameState.pendingAction.targetId !== pending.targetId || this.gameState.pendingAction.type !== pending.type) return;
        let target = this.gameState.players.find(p => p.id === pending.targetId);
        if (!target || target.isBot || !target.alive) return;
        this.log(`⌛ ${target.name} 响应超时，自动跳过`);
        let choice = this.getTimeoutChoice(pending);
        this.resolveResponse(target.id, choice);
    },

    getTimeoutChoice: function(pending) {
        if (pending.type === 'DRAW_CHOICE') return 'DRAW2';
        if (pending.type === 'EXPLOSION') return 'TOP3';
        return 'NO';
    },

    forceTurnTimeout: function(playerId) {
        let p = this.gameState.players.find(x => x.id === playerId);
        if (!p || p.isBot || !p.alive || this.gameState.turnIdx !== p.id || this.gameState.pendingAction || this.gameState.aoeState) return;
        this.log(`⌛ ${p.name} 出牌超时，自动结束回合`);
        let limit = this.getHandLimit(p);
        while (p.hand.length > limit) {
            let c = p.hand.shift();
            this.log(`${p.name} 超时弃掉 ${c.name}`);
        }
        this.gameState.discardingPlayerId = null;
        this.handleActionInternal(p, { type: 'END_TURN' });
    },
    ...
    renderGame: function(state) {
        ...
        if (this.mode === 'MP' && typeof Net !== 'undefined') Net.updateQualityPanel();
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => Game.init(), { once: true });
else Game.init();
